    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if missing_client_cols:
                messages.error(request, f"Missing client columns: {', '.join(missing_client_cols)}")
                return redirect("dashboard")
            if missing_vehicle_cols:
                messages.error(request, f"Missing vehicle columns: {', '.join(missing_vehicle_cols)}")
                return redirect("dashboard")
    ...
    def process_large_file(self, file, request):
        try:
            ...
            if missing_client_cols:
                ...
                return redirect("dashboard")
            if missing_vehicle_cols:
                messages.error(request, f"Missing vehicle columns: {', '.join(missing_client_cols)}")
                return redirect("dashboard")

            bytes_per_row = sample.memory_usage(index=False, deep=True).sum() / len(sample)
